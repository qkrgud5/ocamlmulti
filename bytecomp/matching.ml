  ...
  Llet(Strict, idarg, arg,
       Llet(Alias, tag, Lprim(Pccall prim_obj_tag, [varg]),
            Lifthenelse(
              ...
              Lprim(Pintcomp Ceq,
                    [Lvar tag; Lconst(Const_base(Const_int (Obj.forward_tag-Misc.nst_shift)))]),
              Lprim(Pfield 0, [varg]),
              Lifthenelse(
                (* ... if (tag == Obj.lazy_tag) then Lazy.force varg else ... *)
                Lprim(Pintcomp Ceq,
                      [Lvar tag; Lconst(Const_base(Const_int (Obj.lazy_tag-Misc.nst_shift)))]),
         ...
         (Lswitch
            (varg,
             { sw_numconsts = 0; sw_consts = [];
               sw_numblocks = (max Obj.lazy_tag Obj.forward_tag) - Misc.nst_shift + 1;
               sw_blocks =
                 [ ((Obj.forward_tag-Misc.nst_shift), Lprim(Pfield 0, [varg]));
                   ((Obj.lazy_tag-Misc.nst_shift),
                    Lapply(force_fun, [varg], loc)) ];
